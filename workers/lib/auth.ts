...
export async function getSessionUser(env: Env, request: Request): Promise<AuthUser | null> {
	...
	const response = await stub.fetch("https://user-auth/session", {
		method: "POST",
...
export function canAccessMailbox(user: AuthUser, mailboxId: string): boolean {
	return user.role === "admin" || user.email.toLowerCase() === mailboxId.toLowerCase();
}
