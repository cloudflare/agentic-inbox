...
async function hashPassword(password: string, salt?: Uint8Array): Promise<string> { const actualSalt = salt ?? crypto.getRandomValues(new Uint8Array(16)); const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(password), "PBKDF2", false, ["deriveBits"]); const bits = await crypto.subtle.deriveBits({ name: "PBKDF2", salt: actualSalt, iterations: PBKDF2_ITERATIONS, hash: "SHA-256" }, key, 256); return `${bytesToBase64(actualSalt)}:${bytesToBase64(new Uint8Array(bits))}`; }
async function verifyPassword(password: string, stored: string): Promise<boolean> { const [saltText, expectedText] = stored.split(":"); if (!saltText || !expectedText) return false; const actual = await hashPassword(password, base64ToBytes(saltText)); return timingSafeEqual(actual.split(":")[1], expectedText); }
...
function randomToken(): string { const bytes = crypto.getRandomValues(new Uint8Array(32)); return Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join(""); }

export class UserAuthDO extends DurableObject<Env> {
 private initialized = false;
 private init() { if (this.initialized) return; this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS users (email TEXT PRIMARY KEY,name TEXT NOT NULL,role TEXT NOT NULL,status TEXT NOT NULL,password_hash TEXT NOT NULL,created_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS sessions (token TEXT PRIMARY KEY,email TEXT NOT NULL,expires_at INTEGER NOT NULL); CREATE INDEX IF NOT EXISTS idx_sessions_expires ON sessions(expires_at);`); this.initialized = true; }
 private cleanupSessions() { this.ctx.storage.sql.exec("DELETE FROM sessions WHERE expires_at <= ?", Math.floor(Date.now() / 1000)); }
 async fetch(request: Request): Promise<Response> {
  this.init(); this.cleanupSessions(); const url = new URL(request.url); const body = request.method === "POST" ? await request.json().catch(() => ({})) as Record<string, unknown> : {};
  if (url.pathname === "/seed-admin" && request.method === "POST") {
   const email = normalizeEmail(String(body.email ?? "")); const password = String(body.password ?? ""); if (!email || password.length < 8) return Response.json({ error: "Invalid admin credentials" }, { status: 400 });
   const existing = this.ctx.storage.sql.exec("SELECT * FROM users WHERE email = ?", email).toArray()[0] as UserRecord | undefined;
   const passwordHash = await hashPassword(password);
   if (!existing) this.ctx.storage.sql.exec("INSERT INTO users (email,name,role,status,password_hash,created_at) VALUES (?,?,?,?,?,?)", email, "Administrator", "admin", "active", passwordHash, new Date().toISOString());
   else this.ctx.storage.sql.exec("UPDATE users SET name='Administrator', role='admin', status='active', password_hash=? WHERE email = ?", passwordHash, email);
  ...
  if (url.pathname === "/admin/reset-password" && request.method === "POST") {
   const email = normalizeEmail(String(body.email ?? "")); const password = String(body.password ?? ""); const name = String(body.name ?? "").trim() || email.split("@")[0];
   if (!email.includes("@")) return Response.json({ error: "Invalid email address" }, { status: 400 });
   if (password.length < 8) return Response.json({ error: "Password must be at least 8 characters" }, { status: 400 });
   const passwordHash = await hashPassword(password);
   const existing = this.ctx.storage.sql.exec("SELECT email FROM users WHERE email=?", email).toArray()[0];
   if (existing) this.ctx.storage.sql.exec("UPDATE users SET password_hash=?, status='active', role='employee' WHERE email=?", passwordHash, email);
   else this.ctx.storage.sql.exec("INSERT INTO users (email,name,role,status,password_hash,created_at) VALUES (?,?,?,?,?,?)", email, name, "employee", "active", passwordHash, new Date().toISOString());
