...
app.get("/api/v1/mailboxes/:mailboxId/emails/:emailId/attachments/:attachmentId", async (c: AppContext) => {
	...
	// Fix: Respect disposition field for inline images vs attachments
	// Inline images (with content_id) should return inline, others should be attachment
	const disposition = attachment.disposition === "inline" ? "inline" : "attachment";
	headers.set("Content-Disposition", `${disposition}; filename="${sanitized}"; filename*=UTF-8''${encodeURIComponent(attachment.filename)}`);
