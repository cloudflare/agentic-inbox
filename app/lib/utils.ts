...
function decodeHtmlEntities(text: string): string {
	return text
		...
		.replace(/&#x([0-9a-f]+);/gi, (_match: string, hex: string) => String.fromCharCode(Number.parseInt(hex, 16)))
...
export function getSnippetText(snippet?: string | null, maxLength = 100): string {
	if (!snippet) return "";
	const clean = decodeHtmlEntities(snippet.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "").replace(/<style[^>]*>[\s\S]*/gi, "").replace(/<[^>]*>/g, " ").replace(/<[^>]*$/g, "")).replace(/\s+/g, " ").trim();
	if (!clean) return "";
...
export function getSignatureBlock(settings?: { signature?: { enabled: boolean; text?: string; html?: string } }): string {
...
export function buildQuotedReplyBlock(dateStr: string | undefined, sender: string, body: string): string {
	if (!body) return "";
...
/** Rewrite every Content-ID reference to our same-origin attachment endpoint. */
export function rewriteInlineImages(
	...
	for (const att of attachments) {
		...
		if (!cid) continue;
		const relativeUrl = `/api/v1/mailboxes/${encodeURIComponent(mailboxId)}/emails/${encodeURIComponent(emailId)}/attachments/${encodeURIComponent(att.id)}`;
		const url = typeof window !== "undefined" ? new URL(relativeUrl, window.location.origin).href : relativeUrl;
		const escapedCid = cid.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
		result = result.replace(new RegExp(`cid:\\s*<?${escapedCid}>?`, "gi"), url);
	}
	return result;
}
