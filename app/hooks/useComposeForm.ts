...
const MAX_TOTAL_ATTACHMENT_SIZE = 24 * 1024 * 1024;
		...
		try {
			...
			for (const file of Array.from(files)) {
				if (file.size > MAX_ATTACHMENT_SIZE) {
					toastManager.add({
						title: `File too large: ${file.name} exceeds 20MB limit`,
						variant: "error",
					});
					continue;
				}
				...
				if (total > MAX_TOTAL_ATTACHMENT_SIZE) {
					toastManager.add({
						title: `Total attachment size exceeds 24MB limit`,
						variant: "error",
					});
					break;
				}
			...
			const msg = (err instanceof Error ? err.message : null) || "Failed to read files";
			setError(msg);
