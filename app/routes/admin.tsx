...
export default function AdminRoute() {
	...
	const load = async () => {
		try { const [userData, branding] = await Promise.all([adminApi.listUsers(), adminApi.getBranding()]); setUsers(userData.users); setBrandName(branding.appName); setError(null); } catch (err) { setError(err instanceof Error ? err.message : "Unable to load administration data"); }
	...
	return (
		<div className="min-h-screen bg-kumo-recessed p-6 md:p-10">
			<div className="mx-auto max-w-5xl space-y-8">
				<div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">Administration</h1><p className="mt-1 text-sm text-kumo-subtle">Manage users, branding and the login screen.</p></div><Button variant="secondary" onClick={() => void load()}>Refresh</Button></div>
				...
				<div className="rounded-xl border border-kumo-line bg-kumo-base p-6">
					<h2 className="text-lg font-semibold">Branding</h2><p className="mt-1 text-sm text-kumo-subtle">This overrides the APP_NAME deployment default and is stored with the mailbox system.</p>
				...
				<div className="rounded-xl border border-kumo-line bg-kumo-base p-6">
					<h2 className="text-lg font-semibold">Initialize an existing mailbox</h2><p className="mt-1 text-sm text-kumo-subtle">For mailboxes created before employee login accounts existed. Set a password here; the mailbox itself is not changed.</p>
					<div className="mt-5 grid gap-4 md:grid-cols-3"><Input label="Mailbox email" type="email" value={legacyEmail} onChange={(e) => setLegacyEmail(e.target.value)} placeholder="gavin@astratradehk.com" /><Input label="Name (optional)" value={legacyName} onChange={(e) => setLegacyName(e.target.value)} placeholder="Gavin" /><Input label="Login password" type="password" minLength={8} value={legacyPassword} onChange={(e) => setLegacyPassword(e.target.value)} placeholder="At least 8 characters" /></div>
