...
function rewriteCidImages(body: string, mailboxId?: string, emailId?: string, attachments?: Attachment[]) {
	if (!body || !mailboxId || !emailId || !attachments?.length) return body;
	const urlFor = (att: Attachment) => `/api/v1/mailboxes/${encodeURIComponent(mailboxId)}/emails/${encodeURIComponent(emailId)}/attachments/${encodeURIComponent(att.id)}`;
	let result = body;
	...
	for (const att of attachments) {
		...
		if (re.test(result)) {
			result = result.replace(re, urlFor(att));
			used.add(att.id);
		}
	}

	// Gmail normally puts the original filename in alt. This covers cases where
	// PostalMime did not preserve the Content-ID in the stored attachment row.
	for (const att of attachments) {
		...
		if (re.test(result)) {
			result = result.replace(re, `$1${att.filename}$2${urlFor(att)}$3`);
			used.add(att.id);
		}
	}
	...
	for (let i = 0; i < unresolved.length && i < inlineImages.length; i++) {
		result = result.replace(unresolved[i][0], urlFor(inlineImages[i]));
	}
	return result;
}
...
export default function EmailIframe({ body, mailboxId, emailId, attachments, autoSize }: EmailIframeProps) {
	...
	useEffect(() => {
		...
		const rewrittenBody = rewriteCidImages(body, mailboxId, emailId, attachments);
